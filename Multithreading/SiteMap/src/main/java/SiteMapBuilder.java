...
public class SiteMapBuilder extends RecursiveTask<String> {
    ...
    protected SiteMapBuilder(String url) {
        this.url = url.trim();
    }
    ...
    protected String compute() {
        ...
        List<SiteMapBuilder> writeArrayList = new CopyOnWriteArrayList<>();
        Document document;
        Elements elements;
        try {
            Thread.sleep(150);
            ...
            for (Element element : elements) {
                ...
                if (!attributeUrl.isEmpty() && attributeUrl.startsWith(url) && !WRITE_ARRAY_LIST.contains(attributeUrl) && !attributeUrl
                        .contains("#")) {
                    SiteMapBuilder linkExecutor = new SiteMapBuilder(attributeUrl);
                    linkExecutor.fork();
        ...
        writeArrayList.sort(Comparator.comparing((SiteMapBuilder o) -> o.url));
        int i = 0, allTasksSize = writeArrayList.size();
        while (i < allTasksSize) {
            SiteMapBuilder link = writeArrayList.get(i);
            sb.append(link.join());
            i++;
        }
        return sb.toString();
